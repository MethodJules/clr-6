    ...
    <div v-if="concept" @click="openConcept()" class="conceptMap">
      <div class="showElements">
        ...
        <div class="recomander">
          <v-btn class="addButton" @click="openRecommander">
            Recommander
          </v-btn>
        </div>
      </div>
      <div class="viewConcept">
          ...
          <div class="d-block text-center">
            ...
            <table>
              ...
              <tr>
                ...
                <td>
                  <b-form-input
                    v-model="relationship_label"
                    placeholder="Bitte geben Sie hier die Bezeichnung ein."
                  >
                  </b-form-input>
                </td>
              </tr>
            ...
            <b-button
              class="mt-3"
              block
              @click="createNewConncetionToConceptMap"
              >Speichern</b-button
            >
          </div>
        </b-modal>
        ...
        <b-modal id="create_connection_modal_net" hide-footer>
          <div class="d-block text-center">
            ...
            <b-button
              class="mt-3"
              block
              @click="createNewConncetionToConceptMapFromMap"
              >Speichern</b-button
            >
          </div>
        </b-modal>
        ...
        <b-modal id="create_concept_modal" hide-footer>
          <div class="d-block text-center">
            ...
            <b-form-input
              ...
              placeholder="Bitte geben Sie hier den Concept Namen ein"
            ></b-form-input>
          </div>
        ...
        <b-modal id="create_recommander_modal" hide-footer>
          ...
          <b-button class="mt-3" block @click="createRecommands"
            >erstellen</b-button
          >
        </b-modal>
      </div>
  ...
  methods: {
    ...
    loadTarget() {
      ...
      this.conceptList.forEach((element) => {
        //console.log(element);
        this.target_options.push({
          value: { id: element.nid, uuid: element.id, concept: element.name },
          text: element.name,
        });
      });
    ...
    loadConceptMap() {
      ...
      this.concepts.forEach((element) => {
        this.source_options.push({
          value: { id: element.id, uuid: element.uuid, concept: element.name },
          text: element.name,
        });
      });
    },
        ...
        this.conceptList.forEach((element) => {
          if (element.name === concept) {
            alert(
              "Konzept" +
                concept +
                " existiert bereits und wurde nicht erstellt"
            );
            isEqual = true;
          }
        });
    ...
    createNewConncetionToConceptMap() {
      ...
      if (!isExisting) {
        this.$store.dispatch(
          "concept_map/addConceptToConceptMap",
          this.target_uuid
        ); //TODO: target_uuid ändern in semantisch korrekte bezeichnung
      }
      this.$store.dispatch("concept_map/addRelationshipToConceptMap", {
        source: this.source_uuid,
        target: this.target_uuid,
        name: this.relationship_label,
      });
    ...
    createNewConncetionToConceptMapFromMap() {
      ...
      if (!isExisting) {
        this.$store.dispatch(
          "concept_map/addConceptToConceptMap",
          this.target_uuid_map
        ); //TODO: target_uuid ändern in semantisch korrekte bezeichnung
      }
      this.$store.dispatch("concept_map/addRelationshipToConceptMap", {
        source: this.source_uuid_map,
        target: this.target_uuid_map,
        name: this.relationship_label,
      });
